...
class MM_Site_Build_Status_General_Settings {
  ...
  public function mm_general_settings() {
    ...
    register_setting( $option_group, 'on_off' );
    register_setting( $option_group, 'define_site_build_stages', array( $this, 'define_site_build_stages_validation' ) );

    //add_settings_field( $id, $title, $callback, $page, $section, $args );
    add_settings_field( 'on_off', 'On/Off', array( $this, 'mm_on_off' ), $menu_slug, $page );
    add_settings_field( 'define_site_build_stages', 'Define Site Build Stages', array( $this, 'mm_define_site_build_stages' ), $menu_slug, $page );
  }
  ...
  public function mm_on_off() {
    $options = get_option( 'on_off' );
    $checked = ( @$options == 1 ? 'checked' : '' );
    echo '<label><input type="checkbox" id="on_off" name="on_off" value="1" '.$checked.' /> </label>';
  }

  public function mm_define_site_build_stages() {
    $options = get_option( 'define_site_build_stages' );
  ...
  public function mm_define_site_build_stages_empty_input() {
    ...
    return '<div class="site-build-stage">
        <label>
          <input type="text" name="define_site_build_stages[]" value=""/>
        </label>
        </div>';
  }
  ...
  public function mm_define_site_build_stages_filled_input( $option ) {
      return '<div class="site-build-stage">
        <label>
          <input type="text" name="define_site_build_stages[]" value="' . $option . '"/>' .
